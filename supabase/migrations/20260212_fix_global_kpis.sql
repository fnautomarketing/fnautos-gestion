-- Migration to fix KPI functions for Global Admin View (allow NULL empresa_id)
...
CREATE OR REPLACE FUNCTION public.get_evolucion_facturacion(p_empresa_id uuid, p_fecha_desde date DEFAULT NULL::date, p_fecha_hasta date DEFAULT NULL::date)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
DECLARE
...
CREATE OR REPLACE FUNCTION public.get_estado_facturas(p_empresa_id uuid, p_fecha_desde date DEFAULT NULL::date, p_fecha_hasta date DEFAULT NULL::date)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
DECLARE
...
-- 4. get_top_clientes
CREATE OR REPLACE FUNCTION public.get_top_clientes(p_empresa_id uuid, p_fecha_desde date DEFAULT NULL::date, p_fecha_hasta date DEFAULT NULL::date, p_limite int DEFAULT 10)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
DECLARE
...
CREATE OR REPLACE FUNCTION public.get_facturacion_por_categoria(p_empresa_id uuid, p_fecha_desde date DEFAULT NULL::date, p_fecha_hasta date DEFAULT NULL::date)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
DECLARE
...
-- 6. get_desglose_iva
CREATE OR REPLACE FUNCTION public.get_desglose_iva(p_empresa_id uuid, p_fecha_desde date DEFAULT NULL::date, p_fecha_hasta date DEFAULT NULL::date)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
DECLARE
...
-- 7. get_ranking_conceptos
CREATE OR REPLACE FUNCTION public.get_ranking_conceptos(p_empresa_id uuid, p_fecha_desde date DEFAULT NULL::date, p_fecha_hasta date DEFAULT NULL::date, p_limite int DEFAULT 10)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
  ...
  FROM (
    SELECT
      l.concepto_id,
      l.concepto as concepto_nombre,
      COALESCE(cat.categoria, 'Sin Categoría') as categoria,
      COALESCE(SUM(l.cantidad), 0) as cantidad_vendida,
      COALESCE(SUM(l.subtotal), 0) as ingresos,
      ROUND((SUM(l.subtotal) * 100.0 / NULLIF(SUM(SUM(l.subtotal)) OVER (), 0)), 2) as porcentaje
    FROM lineas_factura l
    JOIN facturas f ON l.factura_id = f.id
    LEFT JOIN conceptos_catalogo cat ON l.concepto_id = cat.id
    WHERE (p_empresa_id IS NULL OR f.empresa_id = p_empresa_id)
      ...
      AND (p_fecha_hasta IS NULL OR f.fecha_emision <= p_fecha_hasta)
      AND f.estado != 'anulada'
    GROUP BY l.concepto_id, l.concepto, cat.categoria
